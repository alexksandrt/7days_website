(() => {
    ...
    function script_headerScroll() {
        ...
        if (scrollPos >= 1) header.classList.add("is-scrolling"); else header.classList.remove("is-scrolling");
    }
    ...
    for (let item of inputs) {
        const input = item.querySelector(".under-input__input");
        input.addEventListener("focusin", (e => {
            item.classList.add("focusing");
        }));
        input.addEventListener("focusout", (e => {
            if (!input.value) item.classList.remove("focusing");
        }));
    }
    const videoPlayButtons = document.querySelectorAll(".responsive-video__play");
    ...
    function YouTubeGetID(url) {
        ...
        if (url[2] !== void 0) ID = url[2]; else ID = url;
        return ID;
    }
    ...
    function generateURL(id) {
        let query = "rel=0&showinfo=0&autoplay=1", sep = id.indexOf("?") > -1 ? "&" : "?";
